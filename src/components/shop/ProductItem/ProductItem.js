      ...
      <div>
        ...
        <p className={styles.PricesWrapper}>
          {!!product.prices.sale ? <>
            ...
            <strong className={styles.NewPrice}>${(product.prices.price - (product.prices.price * product.prices.sale / 100)).toFixed(2)}</strong>
          </> : <strong className={styles.Price}>${(product.prices.price).toFixed(2)}</strong>}
        </p>
      </div>
      {/*TODO сделать компонент кнопку*/}
      <button className={styles.Button}>I'm buying it!</button>
    </div>
  </li>;
};